...
export const createOrder = async (req, res) => {
  const frontendUrl = req.body.frontendUrl || req.headers.origin || req.headers.referer;
  const session = await OrderService.createOrder({ ...req.body, frontendUrl }, req.io);
...
export const createCounterOrder = async (req, res) => {
  const frontendUrl = req.body.frontendUrl || req.headers.origin || req.headers.referer;
  const session = await OrderService.createCounterOrder({ ...req.body, frontendUrl }, req.io);
...
export const createKioskOrder = async (req, res) => {
  ...
  const session = await OrderService.createKioskOrder({ ...req.body, frontendUrl }, clientIp, req.io);
